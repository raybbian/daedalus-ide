...
function updateGLPos(gl: WebGL2RenderingContext | null, pos: [number, number], programs: (WebGLProgram | null)[]) {
	if (gl == null) return;
	programs.forEach((program) => {
		if (program == null) return;
		...
		<div
			...
			onMouseMove={(e) => {
				...
				if (dragging) {
					...
					pos.current = [pos.current[0] + deltaX, pos.current[1] + deltaY];
					updateGLPos(glRef.current, pos.current, [lineProgramRef.current, pixelProgramRef.current]);
					...
					const newMouseGrid = getGridAt([mouseX, mouseY], pos.current, unitLen.current);
					if (curMouseGrid[0] != newMouseGrid[0] || curMouseGrid[1] != newMouseGrid[1]) {
			...
			onWheel={(e) => {
				...
				pos.current = [pos.current[0] + ratio * curMouseGrid[0], pos.current[1] + ratio * curMouseGrid[1]];

				updateGLUnitLen(glRef.current, unitLen.current, [lineProgramRef.current, pixelProgramRef.current]);
				updateGLPos(glRef.current, pos.current, [lineProgramRef.current, pixelProgramRef.current]);
