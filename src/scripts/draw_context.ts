...
export class DrawContext {
	...
	exportToPng(canvasRef: MutableRefObject<HTMLCanvasElement | null>): string | null {
	...
	importFromCanvas(canvasRef: MutableRefObject<HTMLCanvasElement | null>) {
		...
		for (let r = 0; r < imageData.height; r++) {
			for (let c = 0; c < imageData.width; c++) {
				...
				const color = [imageData.data[ind], imageData.data[ind + 1], imageData.data[ind + 2]];
				...
				for (let col = 0; col < 16; col++) {
					...
					if (oColor[0] == color[0] && oColor[1] == color[1] && oColor[2] == color[2]) {
						found = col as PixelColor;
						break;
					}
				}
				if (found == null) {
					console.error("Image has pixels that are not standard daedalus colors");
					return;
